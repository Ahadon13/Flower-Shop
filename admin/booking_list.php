...
include '../includes/pagination.php'; // Include pagination helper
...
// Pagination settings
$recordsPerPage = 10; // Number of bookings per page
...
// Build the query with search conditions (removed user_id restriction)
...
// Build pagination URL with existing parameters
$paginationUrl = '?';
$urlParams = [];
if (!empty($search_query)) $urlParams[] = 'search=' . urlencode($search_query);
if (!empty($date_from)) $urlParams[] = 'date_from=' . urlencode($date_from);
if (!empty($date_to)) $urlParams[] = 'date_to=' . urlencode($date_to);
if (!empty($payment_method)) $urlParams[] = 'payment_method=' . urlencode($payment_method);
if (!empty($event_type)) $urlParams[] = 'event_type=' . urlencode($event_type);
if (!empty($urlParams)) {
...
<div class="customer-orders-details-orders-container">
    ...
    <div class="customer-orders-search-results-info">
        <div class="customer-orders-search-results-text">
            Found <?= $totalRecords ?> booking<?= $totalRecords != 1 ? 's' : '' ?>
            <?php
                    $filters = [];
    ...
    <div class="pagination-info">
        ...
        <strong><?= min($offset + $recordsPerPage, $totalRecords) ?></strong>
        of <strong><?= $totalRecords ?></strong> booking<?= $totalRecords != 1 ? 's' : '' ?>
        <?php if ($totalRecords > $recordsPerPage): ?>
        (Page <strong><?= $currentPage ?></strong> of <strong><?= ceil($totalRecords / $recordsPerPage) ?></strong>)
        <?php endif; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="customer-orders-details-no-orders">
        ...
        <p><?= !empty($search_query) || !empty($date_from) || !empty($date_to) || !empty($payment_method) || !empty($event_type) ? 'No bookings match your search criteria. Try adjusting your filters.' : 'No bookings have been made yet.' ?>
        </p>
        <?php if (!(!empty($search_query) || !empty($date_from) || !empty($date_to) || !empty($payment_method) || !empty($event_type))): ?>
    ...
    <div class="customer-orders-details-order-card">
        <div class="customer-orders-details-order-header" onclick="toggleBooking(<?= $booking['id'] ?>)">
            <div class="customer-orders-details-order-info">
                ...
                <div class="customer-orders-details-order-detail">
                    <div class="customer-orders-details-order-detail-label">Customer</div>
        ...
        <div class="customer-orders-details-order-content" id="content-<?= $booking['id'] ?>">
            ...
            <div class="customer-orders-details-order-details-section">
                ...
                <div class="customer-orders-details-detail-box">
                    ...
                    <p><strong>Customer GCash Number:</strong> <?= htmlspecialchars($booking['gcash_number']) ?></p>
                    <?php endif; ?>
                    <?php if ($booking['gcash_account_name']): ?>
                    <p><strong>Customer Account Name:</strong> <?= htmlspecialchars($booking['gcash_account_name']) ?>
                    </p>
                    <?php endif; ?>
    ...
    <!-- Pagination -->
    <?php renderPagination($totalRecords, $recordsPerPage, $currentPage, $paginationUrl); ?>
    <?php endif; ?>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Reset to page 1 when performing new search
